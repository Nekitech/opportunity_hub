...
const parsePage = async (
    ...
    axios.get(process.env.PARSERS_URL! + "/parsers/"+parser.name+"/"+page)
        .then(async response => {
            ...
            console.error(`[parser:${parser.name}] page ${page} error: ${msg}`);
            try {
                await prisma.accessing_logs_warnings.create({
                    data: {
                        date: new Date(),
                        description: `Parser "${parser.name}" page ${page}: ${msg.slice(0, 500)}`,
                        type: "parser_error",
                        isSolved: false,
                    },
                });
            } catch {
                // don't let logging failure crash the parser loop
            }
        });
}
